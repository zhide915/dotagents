...
`link.ps1` needs Windows Developer Mode (or an elevated shell); it checks and tells you.
...
Paths are 1:1 — `claude/agents/foo.md` lives at `~/.claude/agents/foo.md`. The shared
`instructions.md` is the exception: it deploys as `~/.claude/CLAUDE.md`.
...
Add a top-level folder (e.g. `codex/`) and a `manifest.json` entry pointing it at
that agent's config folder. To share the global instructions, give it an
`"instructions": { "from": "instructions.md", "to": "<that agent's filename>" }` field
(e.g. `AGENTS.md`).
